...
const char *text_of(mrb_state *mrb, mrb_value h, const char *key)
{
    ...
    if (mrb_nil_p(v))
        return nullptr;
    if (!mrb_string_p(v))
        mrb_raisef(mrb, E_ARGUMENT_ERROR, "--%s takes text", key);
...
mrb_int number_of(mrb_state *mrb, mrb_value h, const char *key, mrb_int missing)
{
    ...
    if (mrb_nil_p(v))
        return missing;
    if (!mrb_integer_p(v))
        mrb_raisef(mrb, E_ARGUMENT_ERROR, "--%s takes a whole number", key);
...
int serve(mrb_state *mrb, Invocation &in)
{
    ...
    if (config_path != nullptr) {
        ...
        if (cli_unix == nullptr && cli_port == 0) {
            if (!fc.unix_path.empty())
                cli_unix = fc.unix_path.c_str();
            else if (fc.port != 0)
                cli_port = fc.port;
        }
        if (opts.app_path == nullptr && !fc.app.empty())
            opts.app_path = fc.app.c_str();
        ...
        if (log_path == nullptr && !fc.log_file.empty())
            log_path = fc.log_file.c_str();
        if (log_privacy == nullptr && !fc.log_privacy.empty())
            log_privacy = fc.log_privacy.c_str();
        ...
        if (pidfile == nullptr && !fc.pidfile.empty())
            pidfile = fc.pidfile.c_str();
    ...
    if (pidfile != nullptr) {
        ...
        const int pf = ::open(pidfile, O_WRONLY | O_CREAT | O_TRUNC | O_NOFOLLOW | O_CLOEXEC, 0644);
        char line[32];
    ...
    if (log_max_bytes >= 0)
        opts.log_max_bytes = static_cast<unsigned long long>(log_max_bytes);
    ...
    if (opts.app_path != nullptr) {
        ...
        if (cli_unix != nullptr)
            ...
            taken = "--port";
        else if (opts.standalone_assets_path != nullptr)
            taken = "--assets";
...
void pidfile_remove(const char *path)
{
    ...
    if (n <= 0)
        return;
    buf[n] = '\0';
    if (std::atoi(buf) != static_cast<int>(getpid()))
        return;
    ::unlink(path);
}
...
int main(int argc, char **argv)
{
    ...
    mrb_close(mrb);
    if (in.pidfile != nullptr)
        pidfile_remove(in.pidfile);
    return rc;
}
