...
void die_mdb(const char *what, int rc)
{
    die(what, mdb_strerror(rc));
}
...
std::string ask(const char *prompt)
{
    ...
    if (tty == nullptr)
        die("/dev/tty", std::strerror(errno));
    const int fd = fileno(tty);

    struct termios was {
    };
    if (tcgetattr(fd, &was) != 0)
        die("tcgetattr", std::strerror(errno));
    struct termios now = was;
    now.c_lflag = static_cast<tcflag_t>(now.c_lflag & ~ECHO);
    if (tcsetattr(fd, TCSAFLUSH, &now) != 0)
        die("tcsetattr", std::strerror(errno));
    ...
    if (got == nullptr)
        die("read password", "no input");
    size_t n = std::strlen(buf);
    if (n > 0 && buf[n - 1] == '\n')
        buf[--n] = '\0';
    if (n == 0)
        die("read password", "empty");
    if (n > kMaxPassword)
        die("read password", "longer than 512 bytes");
...
MDB_env *open_env(const char *path, unsigned flags, unsigned maxdbs, size_t mapsize)
{
    ...
    if (rc != 0)
        die_mdb("mdb_env_set_maxdbs", rc);
    rc = mdb_env_set_mapsize(env, mapsize);
    if (rc != 0)
        die_mdb("mdb_env_set_mapsize", rc);
                          ...
                          int64_t mtime)
{
    ...
    if (rc != ARGON2_OK)
        die("argon2_ctx", argon2_error_message(rc));
...
size_t calibrate(double budget_s, unsigned workers, bool say)
{
    ...
    for (size_t i = 0; i < kCostCount; i++) {
        ...
        if (say) {
            std::fprintf(stderr,
                         "  m=%u (%u MiB) t=%u p=%u  %6.1f ms  %6.1f logins/s with %u workers\n",
...
int main(int argc, char **argv)
{
    ...
    if (std::strcmp(cmd, "calibrate") == 0) {
        ...
        if (o.time_ms > 0) {
            std::fprintf(stderr, "\nwithin %.0f ms: m=%u t=%u p=%u\n", o.time_ms,
                         kCosts[pick].m_kib, kCosts[pick].t, kCosts[pick].p);
        }
        return 0;
    }

    if (std::strcmp(cmd, "create") == 0) {
        if (positional[0] == nullptr)
            usage();
    ...
    if (!adding && !setting && !deleting && !listing)
        usage();
    if (positional[0] == nullptr || positional[1] == nullptr)
        usage();
    if (!listing && positional[2] == nullptr)
        usage();
    ...
    if (listing) {
        ...
        for (const Row &row : rows) {
            ...
            char m[32] = "-";
            if (row.rec.ctime != 0)
                spell_time(row.rec.ctime, c, sizeof c);
            if (row.rec.mtime != 0)
                spell_time(row.rec.mtime, m, sizeof m);
    ...
    if (deleting) {
        ...
        if (setting) {
            MDB_val had{};
            if (mdb_get(txn, dbi, &key, &had) == 0 &&
                had.mv_size >= sizeof(webmachine::PasswdRec)) {
        ...
        if (rc == MDB_KEYEXIST)
            die(user, "already there - use set to replace");
        if (rc != 0)
            die_mdb("mdb_put", rc);
    }
