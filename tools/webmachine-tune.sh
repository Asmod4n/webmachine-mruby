...
echo ""
echo "  what the count is for: requests, not bytes. One ring already carries"
echo "  more bytes than any card below 40 Gbit. Measured here, one server,"
echo "  stored assets: 422 MB/s at 4 KiB, 1702 at 32 KiB, 3028 at 256 KiB -"
echo "  24 Gbit on four cpus, and a thread on better silicon carries far"
echo "  more. So a load of large files needs one ring, and a load of many"
echo "  small answers is what $SHAPE_N rings are for: the matrix above is"
echo "  six-byte bodies, which is a request rate and not a byte rate."
if [ "$BUDGET" -le 2 ]; then
