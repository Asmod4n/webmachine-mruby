...
bool config_write_default(const char *path, const char *error_assets)
{
    ...
    std::fprintf(file,
                 ...
                 "\n"
                 "[server]\n"
                 "# Where a files-only server answers. At most one of the two - a unix\n"
                 ...
                 "# port = 8080\n"
                 "\n"
                 "# What it serves: an application, or files. A run that names no\n"
                 "# application serves files. With neither there is no start. An\n"
                 "# application names its own pack and docroot in its conf\n"
                 "# (conf.assets, conf.docroot).\n"
                 "# app = \"site.mrb\"           # the application, as bytecode (mrbc)\n"
                 "# assets = \"site.zip\"        # files only: a pack, answered from one mapping\n"
                 "# docroot = \"/srv/site\"      # files only: a directory of files\n"
                 "\n"
