...
bool conn_has(const char *v, size_t n, const char *lit, size_t litn)
{
    ...
    while (i < n) {
        ...
        while (i < n && v[i] != ',' && v[i] != ' ' && v[i] != '\t')
            i++;
        if (http::tok_eq({v + start, i - start}, {lit, litn}))
            return true;
    }
    return false;
}
...
void read_wire_header(WireSink into, http::Field f)
{
    ...
    if (w.err != 0 || !http::length_is_one_of(nl, kWireLengthMask))
        return;
    switch (nl) {
        case 14:
        ...
        case 10:
            if (http::tok_eq({n, nl}, "user-agent")) {
                ...
                if (conn_has(v, vl, "close", 5))
                    w.conn_close = true;
                else if (conn_has(v, vl, "keep-alive", 10))
                    w.conn_keep = true;
                if (conn_has(v, vl, "upgrade", 7))
                    w.conn_upgrade = true;
            }
            break;
        case 7:
...
void Http1::build_bundle(Bundle &b, const Resource *res)
{
    ...
    b.gzip_ok =
        b.dynamic_body && res->gzip_offered && http::compressible_media_type(b.konst.content_type);
    if (b.gzip_ok) {
...
void Http1::stock_status(bool have[600], uint16_t s)
{
    ...
    if (s == 204 || s == 304)
        ...
        build_status(s, {"", "Content-Length: 0\r\n\r\n"});
    H2Block b;
    h2_build_block(b, {s});
...
void Http1::build(const AppInput *apps, size_t napps)
{
    ...
    for (const auto &f : flow::kFlow) {
        if (f.on_true.status != 0)
            stock_status(have, f.on_true.status);
        if (f.on_false.status != 0)
            stock_status(have, f.on_false.status);
    }
...
void Http1::assemble_dynamic(const DynamicBody &d, std::string &sink)
{
    ...
    if (use_gzip)
        assemble(sink, {d.prefix_gz, gz_body_, d.head_only});
    else
        assemble(sink, {d.prefix_id, d.body, d.head_only});
}
...
Http1::Took Http1::answer_from_assets(Round &r, std::string &sink, Plan *plan)
{
    ...
    if (started_xfer) {
        ...
        if (r.in_place)
            r.st.carry.assign(r.view + r.off, rest);
        else
            r.st.carry.erase(0, r.off);
        if (plan != nullptr) {
...
{
    WantedFile wanted;
    if (!resource_file_wanted(*r.b->res, wanted) || status != 200)
        return false;
...
bool Http1::fail(Conn &st, uint16_t code, std::string &out, uint8_t log)
{
    if (alog_.enabled) {
        log_access(
            alog_,
            {{static_cast<const char *>(st.peer), st.peer_len}, {}, "-", {}, {}, 0, code, log});
    }
...
bool Http1::feed(Conn &st, std::string_view data, Sink out)
{
    ...
    if (mrb_unlikely(st.zc_split) && plan != nullptr)
...
{
    if (st.file == nullptr || st.file->stage != FileStage::kNamed)
        return nullptr;
...
void Http1::file_reject(Conn &st)
{
    file_prebuilt(st, 404);
}
...
void Http1::file_error(Conn &st, const char *why)
{
    log_internal_error(
        elog_,
        {{static_cast<const char *>(st.peer), st.peer_len}, st.file->request_target, why, 500});
...
{
    if (st.file->buf.size() < n)
        st.file->buf.resize(n);
...
void Http1::file_apply(Conn &st, const FileStep &step)
{
    ...
    if (step.head)
        st.file->head.clear(); // the head rides the first round
    if (step.log)
        file_log(st); // before file_clear takes the strings
    if (step.release_map)
        st.map_release();
    if (step.clear)
        st.file_clear();
...
{
    if (!alog_.enabled || st.file == nullptr)
        return;
...
void Http1::bound_prepare(Round &r, const BoundAsk &ask, BoundPrep &prep)
{
    ...
    prep.zc_min =
        (zc_min_ != 0 && plan != nullptr && !st.zc_lent && !head_only && !gz_now) ? zc_min_ : 0;
}
...
Http1::Took Http1::bound_finish(Round &r, const BoundAsk &ask, BoundOut &out)
{
    ...
    if (mrb_unlikely((!b->res->run.content_type.empty() || !ask.rhdrs.empty()) && status != 500)) {
        ...
        if (!bodyless && !head_only) {
            if (lent != nullptr)
                ...
                sink.append(baked ? b->konst.body : ask.body);
        }
        have_body = false;
        answered = true;
    }
                                               ...
                                               Plan *plan, size_t &off)
{
    ...
    if (!st.parked.done()) {
        ...
        if (s.in_place)
            st.carry.assign(s.view + off, rest);
        else
            st.carry.erase(0, off);
...
Http1::Run Http1::run_parkable(Conn &st, RunStart start, std::string *sink, Plan *plan)
{
...

    {
        ...
        uint16_t status = 0;
        if (h1) {
            Round r{st,         b,           s.view,           s.viewlen, s.off,      s.head_len,
                    s.in_place, s.method,    s.method_len,     s.path,    s.path_len, s.minor,
                    s.persist,  s.head_only, s.content_length, s.lflags,  s.facts,    s.vals};
                ...
                ~ParkedRoots()
                {
                    ...
                    for (int i = 0; i < Conn::kJobSlots; i++) {
                        if (round->user_have.at(i))
                            mrb_gc_unregister(res->mrb, round->user_value.at(i));
            ...
            if (mrb_unlikely(refused.status != 0)) {
                ...
                if (h1) {
                    ...
                    H2Stream *const owner =
                        st.h2 != nullptr ? st.h2->find(start.h2.stream_id) : nullptr;
                    if (owner != nullptr) {
                        if (owner->spill.fd >= 0) {
        ...
        if (!h1) {
            ...
            if (stopped)
                h2_log(st, {hq.facts, hq.target});
            co_return sent ? 1 : 0;
        }

        Round fr{st,         b,           s.view,           s.viewlen, s.off,      s.head_len,
                 s.in_place, s.method,    s.method_len,     s.path,    s.path_len, s.minor,
                 s.persist,  s.head_only, s.content_length, s.lflags,  s.facts,    s.vals};
        ...
        const AnswerStep astep =
            spell_answer(fr, {*sink, plan, out.status, out.lent, out.lent_len, out.answered,
                              out.have_body, out.accept_gzip, &b->index, body});
                                 ...
                                 size_t *consumed)
{
    ...
    size_t i = 0;
    while (i < len && seen + i < kH2PrefaceLen && data[i] == kH2Preface[seen + i])
        i++;
...
bool Http1::h1_upgrade_or_stream(Conn &st, const H1Head &h, std::string &sink, bool *lives)
{
    ...
    if (mrb_unlikely(apps_[st.listener].sse_table != nullptr)) {
        ...
        if (sr >= 0) {
            const SseBegin req{sslot,         sr,      h.method,       h.path, sspans,  h.headers,
...
__attribute__((noinline)) static uint16_t head_framing_status(const char *head, size_t len,
                                                              const struct phr_header *hs, size_t n)
{
    for (size_t i = 0; i < n; i++) {
        if (mrb_unlikely(hs[i].name_len == 0))
            return 400;
    }
    for (size_t i = 0; i < len; i++) {
        if (head[i] != '\n')
...
bool Http1::feed_parse(Conn &st, std::string_view in, Sink out)
{
    ...
    Plan *const plan = out.plan;
    if (st.h2 != nullptr)
        return h2_feed(st, in, out);
    if (st.fresh) {
        size_t consumed = 0;
        switch (h1_preface(st, data, len, sink, &consumed)) {
            case Preface::kH2:
                return h2_feed(st, {data + consumed, len - consumed}, out);
            case Preface::kWait:
                return true;
    ...
    switch (st.body_to) {
        case Conn::Body::kNone:
            break;
        case Conn::Body::kMem:
            took = take_body(st, MemWriter{&st.body_hold}, data, len);
            break;
        case Conn::Body::kFile:
            took = take_body(st, FileWriter{&st.spill}, data, len);
            break;
    ...
    if (mrb_unlikely(st.ws != nullptr))
        return ws_feed(st.ws, in, sink);
    if (mrb_unlikely(st.sse != nullptr))
        return true;
    ...
    while (off < viewlen) {
        ...
        // octet lands.
        if (mrb_unlikely(st.spill.bound || st.run_wants_body))
            st.spill.close_file();
        ...
        if (mrb_unlikely(w.err != 0))
            return fail(st, w.err, sink, lflags);
        ...
        if (mrb_unlikely(w.have_te)) {
            ...
            if (mrb_unlikely(!w.te_chunked))
                return fail(st, 501, sink, lflags);
        }
        if (mrb_unlikely(minor >= 1 && !w.have_host))
            return fail(st, 400, sink, lflags);
        ...
        if (mrb_unlikely((w.up_ws && w.conn_upgrade) || apps_[st.listener].sse_table != nullptr)) {
            ...
            bool lives = true;
            if (h1_upgrade_or_stream(st, head, sink, &lives))
                return lives;
        }
        ...
        {
            Round r{st,       nullptr,   view,
                    viewlen,  off,       static_cast<size_t>(ret),
                    in_place, method,    method_len,
                    path,     path_len,  minor,
                    persist,  head_only, w.content_length,
                    lflags,   facts,     vals};
        ...
        if (mrb_unlikely(route < 0)) {
            ...
            if (mrb_likely(b->bound)) {
                ...
                const size_t body_octets = w.content_length != 0 && body_here > w.content_length
                                               ? w.content_length
                                               : body_here;
                ...
                if (mrb_unlikely(b->res->compute != 0 || b->res->watch != 0 ||
                    ...
                    if (mrb_unlikely(r == ComputeRound::kParked))
                        return true;
                    if (mrb_unlikely(r == ComputeRound::kClosed))
                        return false;
                    continue;
                }
                Round br{st,       b,        view,    viewlen,    off + head_len,
                         head_len, in_place, method,  method_len, path,
                         path_len, minor,    persist, head_only,  w.content_length,
                         lflags,   facts,    vals};
                BoundOut bo;
...
bool Http1::ws_upgrade(Conn &st, const WsUpgrade &up, std::string &sink)
{
    ...
    st.content_skip = 0;
    if (!up.rest.empty())
        return ws_feed(st.ws, up.rest, sink);
    return true;
}
