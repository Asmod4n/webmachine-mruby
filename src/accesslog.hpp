...
struct LogRec {
  ...
  uint8_t plen;      // then peer, then target, referer, user-agent
  uint16_t tlen;
  uint16_t rlen;
  uint16_t ulen;
};
inline constexpr uint8_t kLogRecVersion = 1;
...
struct AccessLog {
  ...
  void line(const char* peer, size_t plen, const char* method, size_t mlen, const char* target,
    ...
    if (plen != 0) buf.append(peer, plen);
