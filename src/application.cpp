...
void apply_url(mrb_state *mrb, AppSpec *s, const std::string &u)
{
    ...
    if (unix_form) {
        ...
        if (host.empty())
            mrb_raisef(mrb, E_WM_CONFIG_ERROR(mrb), "conf.url = %s has no host", u.c_str());
    ...
    if (!search.empty()) {
        ...
        for (const auto &kv : params)
            apply_setting(mrb, s, kv.first, kv.second);
    }
}
...
void read_config(mrb_state *mrb, mrb_value conf, AppSpec *s)
{
    ...
    if (conf_str(mrb, conf, kConfUrl, "url", &text))
        apply_url(mrb, s, text);
    ...
    if (mrb_nil_p(mrb_ary_entry(conf, kConfUrl)) && s->form != AppSpec::Form::kNone) {
        ...
        const mrb_value said = s->form == AppSpec::Form::kUnix
...
void register_app(mrb_state *mrb, AppSpec *s)
{
    ...
    for (AppSpec *other : registered_) {
        ...
        if (is_unix) {
            ...
            mrb_raisef(mrb, E_WM_ROUTE_ERROR(mrb),
                       "two applications claim the same listener: port %d", s->port);
        }
    }
    s->registered = true;
    registered_.push_back(s);
...
void application_init(mrb_state *mrb, struct RClass *wm)
{
    ...
    struct RClass *app =
        mrb_define_class_under_id(mrb, wm, MRB_SYM(Application), mrb->object_class);
    ...
    struct RClass *const routes =
        mrb_define_class_under_id(mrb, wm, MRB_SYM(Routes), mrb->object_class);
