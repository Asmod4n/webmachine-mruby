...
// FOUR routes, not one splat. A table with only a splat never runs the
// literal memcmp or a binding capture, so RouteTable::match was half
// unexercised - registration order decides, so the specific ones come
// first and the splat is the fallback it is in a real app.
webmachine::Http1& app() {
  static webmachine::RouteTable* table = [] {
    auto* t = new webmachine::RouteTable();
    t->open(); t->literal("assets", 6); t->splat(); t->commit();
    t->open(); t->literal("users", 5); t->binding(1); t->commit();
    t->open(); t->literal("a", 1); t->literal("b", 1); t->literal("c", 1); t->commit();
    t->open(); t->splat(); t->commit();
    return t;
  }();
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  if (size < 2) return 0;
  // Byte 0 picks the shape; byte 1 is the chunk width, so the SPLIT
  // paths - the carry, a header cut in half, a frame cut in half - are
  // reachable instead of only whole-buffer inputs.
  //
  // Mode 2 exists because HPACK was three layers deep. Reaching
  // lshpack_dec_decode meant discovering a 24-byte preface, then a
  // SETTINGS frame, then a well-formed HEADERS frame header - so the
  // mutator spent its budget in front of the decoder rather than in
  // it, and HPACK is where h2's CVE class lives (decompression bombs,
  // dynamic-table confusion, Huffman overflow). Mode 2 builds all of
  // that scaffolding and hands the input over AS the header block.
  ...
  std::string wire;
  if (mode != 0) wire.assign(kPreface, sizeof(kPreface) - 1);
  if (mode == 2) {
    // Empty SETTINGS, then HEADERS on stream 1 with END_HEADERS |
    // END_STREAM, its whole payload being the fuzzer's bytes.
    static const char kSettings[9] = {0, 0, 0, 4, 0, 0, 0, 0, 0};
    wire.append(kSettings, sizeof(kSettings));
    const size_t n = size < 0xffffff ? size : 0xffffff;
    const char head[9] = {static_cast<char>((n >> 16) & 0xff),
                          static_cast<char>((n >> 8) & 0xff),
                          static_cast<char>(n & 0xff),
                          1,     // HEADERS
                          0x05,  // END_STREAM | END_HEADERS
                          0, 0, 0, 1};
    wire.append(head, sizeof(head));
    wire.append(reinterpret_cast<const char*>(data), n);
  } else {
