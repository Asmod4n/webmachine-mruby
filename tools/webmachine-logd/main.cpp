...
{
    if (max_bytes == 0 || on_disk <= max_bytes)
        return;
    ...
    while (off < len) {
        const ssize_t n =
            ::pwrite(log_fd, tail.data() + start + off, len - off, static_cast<off_t>(off));
        if (n < 0) {
            if (errno == EINTR)
                continue;
...
static void spell_ts(int64_t sec)
{
    ...
    std::snprintf(ts, sizeof ts, "[%02d/%s/%04d:%02d:%02d:%02d +0000]", g.tm_mday, kMon[g.tm_mon],
...
static void spell_peer(const char *sa, size_t salen, bool no_track)
{
    ...
    if (no_track && level == Privacy::kNone)
        level = Privacy::kAnon;
    if (level == Privacy::kFull || salen < 2) {
        out.push_back('-');
        return;
    }
    ...
    for (;;) {
        ...
        while (in.size() - off >= sizeof(LogRec)) {
            ...
            if (r.version != kLogRecVersion) {
                std::fprintf(stderr,
                             "webmachine-logd: access record version %u, built for %u - refusing\n",
                             r.version, kLogRecVersion);
                flush_out();
                std::exit(1);
            }
            const size_t need = sizeof(LogRec) + r.method_token_len + r.peer_len +
                                r.request_target_len + r.referer_len + r.user_agent_len;
            if (in.size() - off < need)
                break;
            const char *p = in.data() + off + sizeof(LogRec);
            const char *method_token = p;
            p += r.method_token_len;
            const char *peer = p;
            p += r.peer_len;
            const char *request_target = p;
            p += r.request_target_len;
            const char *referer = p;
            p += r.referer_len;
            const char *user_agent = p;

            spell_ts(r.unix_seconds);
            if (r.peer_len != 0)
                spell_peer(peer, r.peer_len, (r.flags & kLogNoTrack) != 0);
            else
                out.push_back('-');
            out.append(" - - ", 5);
            out.append(ts, 28);
            out.append(" \"", 2);
            if (r.method_token_len != 0)
                esc(method_token, r.method_token_len);
            else
                out.push_back('-');
            out.push_back(' ');
            ...
            if (r.content_length == 0)
                ...
                spell_num(r.content_length);
            out.append(" \"", 2);
            if (r.referer_len != 0)
                esc(referer, r.referer_len);
            else
                out.push_back('-');
            out.append("\" \"", 3);
            if (r.user_agent_len != 0)
                esc(user_agent, r.user_agent_len);
            else
                out.push_back('-');
            out.append("\"\n", 2);
    ...
    for (;;) {
        ...
        while (in.size() - off >= sizeof(ErrRec)) {
            ...
            if (r.version != kErrRecVersion) {
                std::fprintf(stderr,
                             "webmachine-logd: error record version %u, built for %u - refusing\n",
            ...
            if (parts != r.dynamic_len) {
                std::fprintf(stderr,
                             "webmachine-logd: error record says %u dynamic bytes, its fields add "
            ...
            const char *peer = p;
            p += r.peer_len;
            const char *exception_class = p;
            p += r.exception_class_len;
            const char *request_target = p;
            p += r.request_target_len;
            ...
            out.push_back(' ');
            if (r.peer_len != 0)
                spell_peer(peer, r.peer_len, false);
            else
                out.push_back('-');
            out.push_back(' ');
            if (r.status_code != 0) {
            ...
            out.push_back(' ');
            if (r.method_len != 0)
                esc(method, r.method_len);
            else
                out.push_back('-');
            out.push_back(' ');
            if (r.request_target_len != 0)
                esc(request_target, r.request_target_len);
            else
                out.push_back('-');
            out.push_back(' ');
            if (r.exception_class_len != 0)
                esc(exception_class, r.exception_class_len);
            else
                out.push_back('-');
            out.append(": ", 2);
            if (r.message_len != 0)
                esc(message, r.message_len);
            else
                out.push_back('-');
            out.push_back('\n');
            ...
            if (r.body_full_len != 0) {
                char n[32];
                out.append("\tbody ", 6);
                out.append(n, static_cast<size_t>(std::snprintf(
                                  n, sizeof n, "%u of %u bytes: ", r.body_len, r.body_full_len)));
                esc(body, r.body_len);
                out.push_back('\n');
            }
            out.append("\tbuild ", 7);
...
int main(int argc, char **argv)
{
    ...
    if (argc == 5) {
        if (std::strcmp(argv[4], "anon") == 0)
            privacy = Privacy::kAnon;
        else if (std::strcmp(argv[4], "none") == 0)
            privacy = Privacy::kNone;
        else if (std::strcmp(argv[4], "full") == 0)
            privacy = Privacy::kFull;
        else {
