...
constexpr Pattern kTable[] = {
    ...
    {0, std::string_view("\xfd""7zXZ\x00", 6), "application/x-xz", true},
    ...
    {0, std::string_view("\x00""asm", 4), "application/wasm", false},
    // Fonts.
...
bool known(std::string_view declared) {
  ...
  for (const Pattern& p : kTable) {
    if (same_type(want, p.type)) return true;
  }
  return false;
}
...
bool wants(const std::vector<std::string>& types, std::string_view declared) {
  ...
  for (const std::string& t : types) {
    if (same_type(want, t)) return true;
  }
  return false;
}
...
size_t bytes_wanted() { return 512; }
...
Verdict check(std::string_view declared, std::string_view head) {
  ...
  for (const Pattern& p : kTable) {
    if (!at(head, p.at, p.bytes)) continue;
    if (same_type(want, p.type)) return Verdict::kAgrees;
    if (declared_known) return Verdict::kContradicts;
    if (!p.container) return Verdict::kContradicts;
  ...
  // say so.
  if (declared_known && head.size() >= bytes_wanted()) return Verdict::kContradicts;
  return Verdict::kUnknown;
}
}  // namespace sniff
}  // namespace webmachine
