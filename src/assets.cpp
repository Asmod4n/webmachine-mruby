    ...
    for (mz_uint i = 0; i < n; i++) {
        ...
        if (!mz_zip_reader_file_stat(&za, i, &st)) {
            mrb_raisef(mrb, E_WM_CONFIG_ERROR(mrb), "%s: entry %d: %s", zip_path,
                       static_cast<int>(i), mz_zip_get_error_string(mz_zip_get_last_error(&za)));
        }
        ...
        if (st.m_method != 0 && st.m_method != MZ_DEFLATED) {
            mrb_raisef(mrb, E_WM_CONFIG_ERROR(mrb),
                       ...
                       zip_path, st.m_filename, static_cast<int>(st.m_method));
        }
        ...
        if (!asset_crc_ok(e.file_data, e.compressed_size, e.uncompressed_size, e.deflated,
                          e.crc32)) {
            mrb_raisef(mrb, E_WM_CONFIG_ERROR(mrb),
                       "%s: %s does not hold the octets the pack claims", zip_path, st.m_filename);
        }
        ...
        if (plain.len != 0) {
            ...
            if (cc_ok)
                alias.cache_control.assign(cc.text, cc.len);
...
{
    if (h.unix_seconds == when.unix_seconds)
        return;
...
void Assets::answer_head(const HeadAsk &ask, std::string &sink)
{
    ...
    switch (status_code) {
        case 200:
            h = &ask.entry.head_200[conn];
            break;
        case 304:
            h = &ask.entry.head_304[conn];
            break;
        case 405:
            h = &s405_[conn];
            break;
